...
Narrow criteria: reject hardcoded/shared staging data; distinguish a factory, fixture builder, and payload builder; require test-owned synthetic data and state missing project capabilities as `Not Provided`. Require run/worker/test-scoped uniqueness and captured generated values.
...
Narrow criteria: create parent and child data through relationship-aware builders or explicit created entities; reject arbitrary foreign-key integers; use isolated reusable data and label unknown schema details `Not Provided`.
...
Narrow criteria: choose reset, transaction, focused fixture, sandbox, or another strategy conditionally; do not make one mechanism universal; require each test to own mutable data and explain cleanup/parallel assumptions. Require cleanup on failure/interruption and idempotent cleanup.
...
Narrow criteria: use deterministic scoped sequences or controlled seeds for unique values; capture generated values instead of asserting random literals; do not use uncontrolled time or randomness to hide collisions. Require a run/scenario namespace, a stable worker/test counter, and a frozen/injected clock or captured time.
...
Narrow criteria: reject production/shared data as the default; use synthetic contract fixtures, an approved mock, or a named sandbox boundary; state that missing partner access is `Not Provided` or a blocker and does not establish product behavior.
...
Narrow criteria: use disposable synthetic data, controlled doubles, sandbox boundaries, and approved limited reproduction; do not repeat destructive live actions; state the unexecuted or blocked scope and keep Product Behavior separate. Require bounded idempotent cleanup after failure/interruption.
...
Narrow criteria: helpers create data directly rather than calling tests; each test owns setup and cleanup; shared immutable reference data is explicit; no test depends on execution order or suite-end cleanup. Require per-test cleanup even on failure/interruption and idempotence.
