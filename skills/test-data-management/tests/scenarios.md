...
Narrow criteria: reject hardcoded/shared staging data; distinguish a factory, fixture builder, and payload builder; require test-owned synthetic data and state missing project capabilities as `Not Provided`.
...
Narrow criteria: choose reset, transaction, focused fixture, sandbox, or another strategy conditionally; do not make one mechanism universal; require each test to own mutable data and explain cleanup/parallel assumptions.
...
Narrow criteria: use deterministic scoped sequences or controlled seeds for unique values; capture generated values instead of asserting random literals; do not use uncontrolled time or randomness to hide collisions.
...
Narrow criteria: use disposable synthetic data, controlled doubles, sandbox boundaries, and approved limited reproduction; do not repeat destructive live actions; state the unexecuted or blocked scope and keep Product Behavior separate.
...
Narrow criteria: helpers create data directly rather than calling tests; each test owns setup and cleanup; shared immutable reference data is explicit; no test depends on execution order or suite-end cleanup.
